...
export const DateRangeSearchModal = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setIsColumnMenuOpen(false); // Reset del menú de columnas
    }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-[95vw] w-full h-full md:max-w-[900px] md:w-[900px] md:max-h-[90vh] md:h-auto p-0 gap-0 flex flex-col overflow-hidden">
              ...
              <div className="p-3 md:p-4 border-b border-gray-200 md:border-b-0">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    {/* Botón de configuración de columnas en móvil */}
                    {searchResults.length > 0 && (
                      <div className="md:hidden relative column-menu-container">
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => setIsColumnMenuOpen(!isColumnMenuOpen)}
                          className="text-billpay-blue hover:bg-billpay-blue/10"
                        >
                        ...
                        {isColumnMenuOpen && (
                          <div className="absolute right-0 top-full mt-1 z-50 bg-white border border-gray-200 rounded-md shadow-lg p-2 w-48">
                            <h4 className="text-xs font-medium text-gray-700 mb-2">Columnas visibles</h4>
                            <div className="space-y-1">
                              <label className="flex items-center text-xs">
                                <input
                                  type="checkbox"
                                  checked={visibleColumns.SerNr}
                                  onChange={() => toggleColumn('SerNr')}
                                  className="mr-2"
                                />
                                Nro. Doc
                              </label>
                              <label className="flex items-center text-xs">
                                <input
                                  type="checkbox"
                                  checked={visibleColumns.OfficialSerNr}
                                  onChange={() => toggleColumn('OfficialSerNr')}
                                  className="mr-2"
                                />
                                Nro. CUFE
                              </label>
                              <label className="flex items-center text-xs">
                                <input
                                  type="checkbox"
                                  checked={visibleColumns.FormaPago}
                                  onChange={() => toggleColumn('FormaPago')}
                                  className="mr-2"
                                />
                                Tipo
                              </label>
                              <label className="flex items-center text-xs">
                                <input
                                  type="checkbox"
                                  checked={visibleColumns.InvDate}
                                  onChange={() => toggleColumn('InvDate')}
                                  className="mr-2"
                                />
                                Fecha
                              </label>
                              <label className="flex items-center text-xs">
                                <input
                                  type="checkbox"
                                  checked={visibleColumns.Sum4}
                                  onChange={() => toggleColumn('Sum4')}
                                  className="mr-2"
                                />
                                Importe
                              </label>
                              <label className="flex items-center text-xs">
                                <input
                                  type="checkbox"
                                  checked={visibleColumns.OrderNr}
                                  onChange={() => toggleColumn('OrderNr')}
                                  className="mr-2"
                                />
                                Orden
                              </label>
                              <label className="flex items-center text-xs">
                                <input
                                  type="checkbox"
                                  checked={visibleColumns.CustOrdNr}
                                  onChange={() => toggleColumn('CustOrdNr')}
                                  className="mr-2"
                                />
                                Orden Cliente
                              </label>
                              <label className="flex items-center text-xs">
                                <input
                                  type="checkbox"
                                  checked={visibleColumns.RefStr}
                                  onChange={() => toggleColumn('RefStr')}
                                  className="mr-2"
                                />
                                Referencia
                              </label>
            ...
            <div className="flex-1 overflow-hidden">
              {searchResults.length > 0 ? (
                <div className="h-full rounded-md border overflow-auto">
                  <div className="w-full md:min-w-0 min-w-[800px]">
                    <Table className="w-full table-auto">
                      <TableHeader>
                        <TableRow>
                          {visibleColumns.SerNr && (
                            <TableHead className="text-xs md:text-sm md:pl-2 px-2 whitespace-nowrap md:w-auto w-20">
                              <Button
                                variant="ghost"
                                size="sm"
                          ...
                          {visibleColumns.OfficialSerNr && (
                            <TableHead className="text-xs md:text-sm px-2 whitespace-nowrap md:w-auto w-24">
                              Nro. CUFE
                            </TableHead>
                          )}
                          {visibleColumns.FormaPago && (
                            <TableHead className="text-xs md:text-sm px-2 whitespace-nowrap md:w-auto w-20">Tipo</TableHead>
                          )}
                          {visibleColumns.InvDate && (
                            <TableHead className="text-xs md:text-sm px-2 whitespace-nowrap md:w-auto w-20">
                              <Button
                                variant="ghost"
                                size="sm"
                          ...
                          {visibleColumns.Sum4 && (
                            <TableHead className="text-right text-xs md:text-sm px-2 whitespace-nowrap md:w-auto w-24">
                              Importe
                            </TableHead>
                          )}
                          {visibleColumns.PDF && (
                            <TableHead className="text-center text-xs md:text-sm px-2 whitespace-nowrap md:w-auto w-16">
                              PDF
                            </TableHead>
                          )}
                          {visibleColumns.OrderNr && (
                            <TableHead className="text-xs md:text-sm px-2 whitespace-nowrap md:w-auto w-20">Orden</TableHead>
                          )}
                          {visibleColumns.CustOrdNr && (
                            <TableHead className="text-xs md:text-sm px-2 whitespace-nowrap md:w-auto w-28">
                              Orden Cliente
                            </TableHead>
                          )}
                          {visibleColumns.RefStr && (
                            <TableHead className="text-xs md:text-sm px-2 whitespace-nowrap md:w-auto w-24">
                              Referencia
                            </TableHead>
                          )}
                        ...
                        {searchResults.length === 0 ? (
                          ...
                          searchResults.map((invoice) => (
                            <TableRow key={invoice.id}>
                              {visibleColumns.SerNr && (
                                <TableCell className="text-xs md:text-sm font-mono md:pl-2 px-2">
                                  {invoice.SerNr}
                                </TableCell>
                              )}
                              {visibleColumns.OfficialSerNr && (
                                <TableCell className="text-xs md:text-sm font-mono px-2">
                                  {invoice.OfficialSerNr}
                                </TableCell>
                              )}
                              {visibleColumns.FormaPago && (
                                <TableCell className="text-xs md:text-sm px-2">
                                  {invoice.FormaPago}
                                </TableCell>
                              )}
                              {visibleColumns.InvDate && (
                              ...
                              {visibleColumns.Sum4 && (
                                <TableCell
                                  className={`font-medium text-right text-xs md:text-sm px-2 ${
                                    invoice.FormaPago === 'Nota de Crédito' ? 'text-red-600' : ''
                                  }`}
                                >
                                  {invoice.FormaPago === 'Nota de Crédito'
                ...
                <div className="space-y-2 px-4 py-6 text-center text-sm text-gray-500">
